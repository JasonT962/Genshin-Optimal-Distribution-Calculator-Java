...
public class AlhaithamSpreadOptimizer {
    private Player player;
    private AlhaithamCalculator calculator;
    ...
    private double addRolls = 5;

    public AlhaithamSpreadOptimizer(AlhaithamCalculator calculator) {
        player = new Player();
    ...
    public double MaxDamage(Artifact[] set) {
        ...
        for (int i = 0; i < 25; i++) {
            player.hpUp += rollHP;
            double damage1 = calculator.SpreadAverageDamage(player);
            player.hpUp -= rollHP;

            player.flatHp += rollFlatHP;
            double damage2 = calculator.SpreadAverageDamage(player);
            player.flatHp -= rollFlatHP;

            player.attackUp += rollAttack;
            double damage3 = calculator.SpreadAverageDamage(player);
            player.attackUp -= rollAttack;

            player.flatAttack += rollFlatAttack;
            double damage4 = calculator.SpreadAverageDamage(player);
            player.flatAttack -= rollFlatAttack;

            player.defenceUp += rollDefence;
            double damage5 = calculator.SpreadAverageDamage(player);
            player.defenceUp -= rollDefence;

            player.flatDefence += rollFlatDefence;
            double damage6 = calculator.SpreadAverageDamage(player);
            player.flatDefence -= rollFlatDefence;

            player.critRate += rollCR;
            double damage7 = calculator.SpreadAverageDamage(player);
            player.critRate -= rollCR;

            player.critDamage += rollCD;
            double damage8 = calculator.SpreadAverageDamage(player);
            player.critDamage -= rollCD;

            player.elementalMastery += rollEM;
            double damage9 = calculator.SpreadAverageDamage(player);
            player.elementalMastery -= rollEM;

            player.energyRecharge += rollER;
            double damage10 = calculator.SpreadAverageDamage(player);
            player.energyRecharge -= rollER;
        ...
        return calculator.SpreadAverageDamage(player);
    }
    ...
    public void printStats(Artifact[] set) {
        ...
        System.out.println("Average Damage: "+calculator.SpreadAverageDamage(player));
        System.out.println();
